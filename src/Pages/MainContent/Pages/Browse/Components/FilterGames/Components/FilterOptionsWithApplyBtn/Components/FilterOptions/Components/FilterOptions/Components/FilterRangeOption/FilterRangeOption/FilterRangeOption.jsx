...
export default function FilterRangeOption({
  ...
  const disableState =
    typeof limit !== "object" || limit.higher <= limit.lower || disabled;
  const bigger = parseFloat(
    (Math.max(knob1, knob2) / singleStepRef.current + limit.lower).toFixed(
      float,
    ),
  );
  ...
  const smaller = parseFloat(
    (
      (Math.min(knob1, knob2) / singleStepRef.current || 0) + limit.lower
    ).toFixed(float),
  );
