...
export default function useGameInfoFieldLogics({ gameData, errorMessages }) {
  ...
  if (!eventRefs.current) {
    eventRefs.current = {
      checkValidation: () => {
        ...
        if (
          typeof gameInfo.phoneLogo === "object"
            ? !gameInfo.phoneLogo.file
            : !gameInfo.phoneLogo
        ) {
        ...
        if (
          !gameInfo.releaseDate.day ||
          !gameInfo.releaseDate.month ||
          !gameInfo.releaseDate.year
        ) {
        ...
        errorMessages.current.gameDescriptionsError.descriptions =
          gameDescriptions.descriptions.map((desc) => {
            const obj = {};
            if (desc.mainHeader === "") {
        ...
        if (
          ...
          gameSpecificationsError.spec[3] =
            "At least One System Requirements is required";
          error = true;
        } else {
          if (gameSpecificationsError.spec[3]) {
            gameSpecificationsError.spec.pop();
          }
          ...
          for (let i = 0; i < 3; i++) {
            ...
            if (spec.isActive) {
              ...
              for (let k = 0; k < 2; k++) {
                ...
                const check = systemReqMustArr.filter(
                  (la) => !mustReqARr.includes(la),
                );
                ...
                if (check.length) {
                  ...
                  error = true;
                  gameSpecificationsError.spec[i][k ? "rec" : "min"] =
                    `${result} ${length > 1 ? "are" : "is"} must be filled`;
                } else {
                  gameSpecificationsError.spec[i].req[k ? "rec" : "min"] =
                    mustReqARr.map((...rest) =>
        ...
        if (Array.isArray(gameSpecifications.others.value)) {
          if (!gameSpecifications.others.value[0]) {
            ...
            gameSpecificationsError.others[0] =
              "Text Language Supported is required";
          } else {
            gameSpecificationsError.others[0] = "";
          }
          if (!gameSpecifications.others.value[1]) {
            ...
            gameSpecificationsError.others[1] =
              "Audio Language Supported is required";
          } else {
            gameSpecificationsError.others[1] = "";
          }
      ...
      handleUnnecessaryRemove: () => {
        ...
        cleanData.gameSpecifications.spec =
          cleanData.gameSpecifications.spec.filter((spec) => spec.isActive);
        cleanData.gameSpecifications.spec =
          cleanData.gameSpecifications.spec.map((spec) => {
            const newSpec = { ...spec };
  ...
  return {
    checkValidation: eventRefs.current.checkValidation,
    handleUnnecessaryRemove: eventRefs.current.handleUnnecessaryRemove,
  };
}
