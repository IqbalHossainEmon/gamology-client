...
export default function GameInfoField({
  ...
  const handleSubmit = (e) => {
    ...
    if (
      errorMessages.current.outerErrorMessage !==
      errorMessages.current.isThereError
    ) {
      setErrorChange((prev) => prev + 1);
    }
  ...
  useEffect(() => {
    ...
    if (defaultData && hasDefault && Object.keys(defaultData).length) {
      ...
      const specList = defaultGameData.gameSpecifications.spec.map(
        (spec) => spec.for,
      );
      const defSpec = {
        systemReq: [
      ...
      gameData.current.gameTags = categories.reduce((acc, category) => {
        const obj = {};
        const defaultCategory = hasDefault
          ? defaultData.gameTags[category]
          : {};
