...
export default function useGameInfoFieldLogics({ gameData, errorMessages }) {
	const checkValidation = () => {
		const { gameInfo, gameBanner, gameDescriptions, gameSpecifications, gameTags } =
			gameData.current;
		const { gameInfoError, gameDescriptionsError, gameSpecificationsError, gameTagsError } =
		...
		if (
			typeof gameInfo.phoneLogo === 'object' ? !gameInfo.phoneLogo.file : !gameInfo.phoneLogo
		) {
				...
				if (spec.isActive) {
					...
					for (let k = 0; k < 2; k++) {
						...
						if (check.length) {
							...
							gameSpecificationsError.spec[i].req[k ? 'rec' : 'min'] = mustReqARr.map(
								(...rest) => handleFieldCheck(systemReq, k, ...rest)
							);
						}
					}
				}
