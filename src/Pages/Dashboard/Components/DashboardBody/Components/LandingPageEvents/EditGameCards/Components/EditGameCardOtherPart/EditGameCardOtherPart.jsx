...
function EditGameCardOtherPart({
  gamesCards,
  cardsRef,
  setGameCards,
  errorMessages,
  ...
  const handleMoveLeftRight = (id, outerIndex, index, isLeft) => {
    ...
    const temp = cardsRef.current[outerIndex].cards[index].cards.splice(
      cardIndex,
      1,
    )[0];
      ...
      {gamesCards.map((gameCards, outerIn) => {
        ...
        return (
          <div key={gameCards.id}>
            ...
            <div
              key={gameCards.id}
              className={styles.editGameCardOtherPartContainer}
            >
                  ...
                  <EditGameCardContainer
                    ...
                    onIndividualDelete={(id) => {
                      cardsRef.current[outerIndex].cards[index].cards =
                        cardsRef.current[outerIndex].cards[index].cards.filter(
                          (card) => card.id !== id,
                        );
                    }}
                    ...
                    setHeader={(header) => {
                      cardsRef.current[outerIndex].cards[index].header = header;
                    }}
                    setGameCard={(card) => {
                    ...
                    onDelete={(e) => {
                              ...
                              <ButtonWithRipple
                                onClick={() => {
                                  setGameCards((prev) => {
                                    ...
                                    cardsRef.current.splice(outerIndex, 1);
                                    return newPrev;
                                  });
                                  hideModal();
                                }}
                      ...
                      return () =>
                        setGameCards((prev) => {
                          ...
                          cardsRef.current[outerIndex].cards.splice(index, 1);
                          return newPrev;
                        });
                    }}
                  />
