...
function EditGameCardFirstPart({
  gameCards,
  setGameCards,
  cardsRef,
  errorMessages,
  ...
  const handleMoveLeftRight = (id, index, isLeft) => {
    const cardIndex = cardsRef.current[0].cards[index].cards.findIndex(
      (card) => card.id === id,
    );
            ...
            <EditGameCardContainer
              ...
              onIndividualDelete={(id) => {
                cardsRef.current[0].cards[index].cards =
                  cardsRef.current[0].cards[index].cards.filter(
                    (card) => card.id !== id,
                  );
              }}
              ...
              onDelete={(e) => {
                if (cardsRef.current[0].cards.length === 1) {
                  setContent({
                    ...
                    body: (
                      <p>
                        You can not Delete the last game cards after the main
                        banner
                      </p>
                    ),
                    footer: (
