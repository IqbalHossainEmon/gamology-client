  ...
  if (!eventRefs.current) {
    eventRefs.current = {
      onClick: () => {
        ...
        if (Array.isArray(dataHolder.current)) {
          ...
          if (!dataHolder.current.discount) {
            ...
            errorMessage.current.discount =
              "Discounted price must be a number.";
            isThereError = true;
          } else {
            errorMessage.current.discount = "";
          }
          if (
            ...
            Number(dataHolder.current.discount) >=
              Number(dataHolder.current.regular)
          ) {
            errorMessage.current.discount =
        ...
        if (isThereError) {
          ...
          setAdaptiveGameCards((prev) => {
            ...
            newAdaptiveGameCards[index].footer = cloneObject(
              dataHolder.current,
            );
      ...
      setValue: (val, nameWithIndex) => {
        const [name, innerIndex] = nameWithIndex
          ? nameWithIndex.split("-")
          : [];
        if (innerIndex) {
  ...
  return (
    <>
      <p className={styles.editFooterHeader}>
        Edit the footer of card {index + 1}?
      </p>
