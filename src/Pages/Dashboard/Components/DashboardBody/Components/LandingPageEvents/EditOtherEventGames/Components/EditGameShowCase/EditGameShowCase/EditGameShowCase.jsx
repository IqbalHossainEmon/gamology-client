...
function extraCard(index, onclick) {
  return (
    <EditGameShowCaseExtraCard
      index={index}
      onclick={(game) => onclick(index, game)}
    />
  );
}
...
const handleCardDotList = (items, setItems, dataRef, outerIndex) =>
  function Inner(parentRef, item, parentIndex) {
    const getListHandler = () => {
      ...
      const handleMove = (direction) => {
        ...
        setItems((prev) => {
          ...
          index = newItems[parentIndex].games.findIndex(
            (card) => card.id === id,
          );
      ...
      const newList = [
        {
          name: "Delete",
          event: () => {
            const newCards = items[parentIndex].games.filter(
              (card) => card.id !== id,
            );
            setItems((prev) => {
              const newItems = cloneObject(prev);
    ...
    return (
      <GameCardManagementMenu
        parentRef={parentRef}
        item={item}
        lists={getListHandler()}
      />
    );
  };
