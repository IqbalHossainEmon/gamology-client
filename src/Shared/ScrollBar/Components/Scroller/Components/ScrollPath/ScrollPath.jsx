...
function ScrollPath({ container, innerContainer, thumb, height }) {
  ...
  const { widthInRem, remHeightInPixels, heightInRem } = useScreenWidth();
    ...
    pathRef.current?.addEventListener(
      ...
      () => {
        changingValue.current =
          (innerContainer.current.scrollHeight /
  ...
  if (!eventRefs.current) {
    ...
    eventRefs.current = {
      resumeEvent: () => {
        ...
        scroll();
        pathRef.current.removeEventListener(
          "mousemove",
          eventRefs.current.resumeEvent,
        );
  ...
  return (
    <div
      ref={pathRef}
      style={{ height: `${height / remHeightInPixels}rem` }}
