...
function Scroller({ innerContainerRef, outerContainerRef, showPath }) {
  ...
  const [style, setStyle] = useState(
    showPath
      ? { height: 0, factor: 0, scrollerHeight: 0 }
      : { height: 0, factor: 0 },
  );
  ...
  useEffect(() => {
    ...
    const updateThumb = () => {
      ...
      const thumbHeight =
        (containerHeight / scrollerContainer) * containerHeight;
      ...
      let factor =
        (containerHeight - thumbHeight) / (scrollerContainer - containerHeight);
      ...
      if (showPath) {
        setStyle({
          height: thumbHeight,
          factor,
          scrollerHeight: scrollerContainer,
        });
      } else {
