...
function SuggestionList({ state, setShow, setValue, setState, name, value, elementRef }) {
		...
		eventRefs.current = {
			handleCalcPosition: length => {
				...
				if (length > 4) {
					...
					positionRef.current.height = 250;
				} else if (bottomRemain < length * 60 + 10) {
					positionRef.current.bottom = false;
					positionRef.current.height = (length || 1) * 60 + 10;
				} else {
					positionRef.current.bottom = true;
					positionRef.current.height = (length || 1) * 60 + 10;
				}
	...
	useEffect(() => {
		...
		// strong the mated word or part

		const newFilteredList = cloneObject(filteredList);
		handleDebouncing(() => eventRefs.current.handleAddStrongMatch(newFilteredList));
	}, [cloneObject, handleDebouncing, value]);
