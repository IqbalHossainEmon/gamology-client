...
export type TooltipInteractionHandlers = (
	{ current }: RefObject<TooltipRefValue>
) => (container: HTMLElement) => void;

const withTooltip = <T extends object>(Component: ComponentType<T>) =>

	function InnerComponent(props: T) {
		const [tooltips, setTooltips] = useState([] as Tooltip[]);
		const timeIdRef = useRef<ReturnType<typeof setTimeout> | null>(null);
		const idRef = useRef(0);
		const elementOnHideListRef = useRef<{ container: HTMLElement, timerId: ReturnType<typeof setTimeout> }[]>([]);
		...
		const tooltipInteractionHandlers: TooltipInteractionHandlers = useCallback(
			({ current }) => {
				...
				setTooltips(prev => {
					...
					if (index > -1)
						return prev.map((tooltip, i) => i === index ? {
							id: tooltip.id,
							container,
							show: true,
							message,
